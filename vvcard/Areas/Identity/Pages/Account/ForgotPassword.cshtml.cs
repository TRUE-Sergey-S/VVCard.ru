    ...
    public class ForgotPasswordModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            if (ModelState.IsValid)
            {
                ...
                await _emailSender.SendEmailAsync(Input.Email, "Сброс пароля на сайте VVCard.ru",
                        $"Добрый день! {user.UserName} <br> " +
                        $"Для сброса, перейдите по ссылке: <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'><b>Ссылка</b></a>. <br><br><br>" +
